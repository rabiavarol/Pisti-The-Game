    ...
    public List<GameEnvironment> simulateGame(Short cardNo) {
        ...
        gameEnvironmentList.add(
            mGame.createPlayerEnvironment(
                    simulateMovement(cardNo, mGame.getMTurn()), isGameFinished(Game.Side.PLAYER), Game.MoveType.CARD)
        );

        // Simulate pc movement and create game environment
        gameEnvironmentList.add(
                mGame.createPcEnvironment(
                        simulateMovement(pcStrategicDecideCard(isFirstPcMove), mGame.getMTurn()), isGameFinished(Game.Side.PC), Game.MoveType.CARD)
        );
